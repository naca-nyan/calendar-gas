...
function notifyWeekly() { //週間予定通知

	var dt = new Date()
		var message = "来週の天ひま！の予定一覧はこちら↓\n※毎週日曜日に、週間予定をお知らせしています！\n\n";
	...
	for (var i = 0; i < 7; i++) {
		...
		var calendarName = calendar.getId()//カレンダー読み取り
			if (calendarName == undefined) {//カレンダー全体の予定有無判定
				message += "予定はありません\n\n";
				continue;//LP飛ばす
			}

		//var events = calendar.getEventsForDay(dt);//取得日の予定//←重複排除
		if (events.length == 0) {//取得日の予定有無判定
			message += "予定はありません\n\n";
			continue;//LP飛ばす
		}

		for (var j = 0; j < events.length; j++) {//取得日の予定全LP
			dayText += String(Utilities.formatDate(events[j].getStartTime(), "JST", "HH:mm"));//予定開始時刻
			dayText += "-"
				dayText += String(Utilities.formatDate(events[j].getEndTime(), "JST", "HH:mm"));//予定終了時刻
			dayText += " "
